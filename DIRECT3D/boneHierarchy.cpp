...
)
{
	...
	for (DWORD i = 0; i < NumMaterials; i++)
	{
		...
		if (pMaterials[i].pTextureFilename != NULL)
		{
			...
			if( FAILED( D3DXCreateTextureFromFile(d3dDevice, texPath.c_str(), &pTex ) ) )
			{
			//Texture 푸쉬
			...
			boneMesh->DiffuseTexs.push_back(RM_TEXTURE->addResource(texPath));
			...
			if (pNorTex == NULL)
			{
				pNorTex = RM_TEXTURE->addResource("Resources/Mesh/normalDefault.png");
			}
			...
			LPDIRECT3DTEXTURE9 pEmiTex = RM_TEXTURE->addResource(texFilePath);
			//만약 Texture 가 없다면...
			if (pEmiTex == NULL)
			{
				pEmiTex = RM_TEXTURE->addResource("Resources/Mesh/emissionDefault.png");
			}
			...
			LPDIRECT3DTEXTURE9 pSpcTex = RM_TEXTURE->addResource(texFilePath);
			//만약 Texture 가 없다면...
			if (pSpcTex == NULL)
			{
				pSpcTex = RM_TEXTURE->addResource("Resources/Mesh/specularDefault.png");
			}
			//Texture 푸쉬
