...
void timeManager::render(void)
{
	...
	//  FPS
	sprintf(str, "FPS : %d", _frameRate);
	FONTMANAGER->fontOutLine(str, 0, 0, 0xffffff00);

	//  한 프레임당 경과시간
	sprintf(str, "DeltaTome : %.2f", _timeElapsed);
	FONTMANAGER->fontOutLine(str, 0, 25, 0xffffff00);

	//  전체 시간
	sprintf(str, "WorldTime : %.2f", _worldTime);
	FONTMANAGER->fontOutLine(str, 0, 50, 0xffffff00);
